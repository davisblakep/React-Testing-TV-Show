...
test("Successfully renders API data", async () => {
    mockFetchShow.mockResolvedValueOnce(showData, seasonData)
    ...
    const seasonOneSelect = await findByText(/season 1/i);
    userEvent.click(seasonOneSelect);


    await waitFor(() => {
        expect(getAllByTestId(/episode/)).toHaveLength(8);
    })
